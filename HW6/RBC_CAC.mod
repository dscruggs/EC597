//Scruggs: April 12th; RBC model with Capital adjustment costs.
...
model;
    w = ttheta*c*n^ggamma;
    bbeta*(c/c(+1))*Rf = 1;
    q=bbeta*c/c(+1)*(r(+1)+q(+1)*((1-ddelta)-pphi/2*(inv(+1)/k-ddelta)^2+pphi*(inv(+1)/k-ddelta)*inv(+1)/k));
    r=aalpha*y/k(-1);
    w=(1-aalpha)*y/n;
    y=A*k(-1)^aalpha*(n)^(1-aalpha);
    c+inv=y;
    k=(1-ddelta)*k(-1)+inv+(pphi/2)*(inv/k(-1)-ddelta)^2;
    q = 1/(1-pphi*(inv/k(-1)-ddelta));
    //shock
